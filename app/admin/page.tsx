...
// 2. PRODUCTS VIEW (Advanced Edit & Add)
const ProductsView = () => {
  ...
  const initialProductState: ExtendedProduct = {
    ...
    name: "",
    brand: "",
    price: 0, // Selling Price (Discounted)
    originalPrice: 0, // Real Price
  ...
  const [imageInput, setImageInput] = useState(""); // For multiple image input string
  ...
  const handleOpenEdit = (product: ExtendedProduct) => {
    ...
    // Join images array back to string for textarea
  ...
  const handlePriceCalculation = (sellingPrice: number, discount: number) => {
    // Logic: Real Price = Selling Price / (1 - Discount/100)
    // Example: 8500 / (1 - 0.15) = 10000
...
// 4. GENERIC LIST VIEW (For Categories, FAQs, etc.)
...
const GenericListView = ({ title, data, onAdd, onDelete, fields, idKey }: ListViewProps) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
        ...
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 rounded-r-md">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-blue-600 text-xs mt-1">Click 'Export Data' to download the updated code file.</p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="flex min-h-screen bg-[#F9F9F9] font-sans text-[#333]">
      ...
      <main className="flex-1 p-4 md:p-8 md:ml-64 pt-20 md:pt-8 w-full max-w-full overflow-hidden">
        ...
        {/* REUSED GENERIC VIEWS FOR OTHER TABS */}
        {activeTab === "Featured" && (
          <GenericListView 
