...
function ProductsContent() {
  const { products, categories, addToCart, cart, setIsCartOpen, toggleWishlist, wishlist, user } = useGlobalContext();
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col font-sans text-[#333]">
      ...
      <div className="sticky top-0 z-50 bg-white shadow-sm">
            ...
            <div className="flex items-center gap-5 text-zinc-200 ml-auto md:ml-0">
               <div ref={searchContainerRef} className="flex items-center relative">
                  <div className={`transition-all duration-500 ease-in-out overflow-hidden flex items-center ${isSearchOpen ? "w-40 md:w-64 opacity-100 mr-2" : "w-0 opacity-0 mr-0"}`}>
                    <input type="text" placeholder="Search brand, name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white text-black text-xs px-3 py-1.5 rounded-sm border-none focus:ring-2 focus:ring-gray-500 outline-none" autoFocus={isSearchOpen} />
                  </div>
                  <button onClick={() => setIsSearchOpen(!isSearchOpen)} className={`hover:text-white cursor-pointer transition-colors ${isSearchOpen ? 'text-white' : ''}`} title="Search">
               ...
               <button onClick={handleUserClick} className="hover:text-white cursor-pointer transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A7.5 7.5 0 0 1 4.501 20.118Z" /></svg>
               </button>
          ...
          {/* C. SORTING & BREADCRUMB BAR */}
          <div className="container mx-auto px-4 py-4 border-b border-gray-100 bg-white">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="text-xs text-gray-500 font-medium">
                <Link href="/" className="hover:text-black">Home</Link> <span className="mx-2">›</span> <span className="text-black font-bold">All Products</span>
              ...
              <div className="flex items-center justify-between w-full md:w-auto gap-6">
                ...
                <div className="flex items-center gap-6 text-sm ml-auto md:ml-0">
                    ...
                    <div className="relative">
                        <button onClick={() => setIsSortOpen(!isSortOpen)} className="flex items-center gap-2 font-bold text-black hover:text-gray-700 transition-colors">
                            {sortOption === "default" ? "Default Sorting" : sortOption === "lowToHigh" ? "Price: Low to High" : "Price: High to Low"}
                            <svg width="10" height="6" viewBox="0 0 10 6" fill="none" className={`transform transition-transform duration-200 ${isSortOpen ? "rotate-180" : ""}`}><path d="M1 1L5 5L9 1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                        </button>
                        {isSortOpen && (
                            <>
      ...
      <div className="flex container mx-auto px-4 md:px-0 mb-10">
        ...
        <aside className={`fixed inset-y-0 left-0 z-50 lg:z-30 w-[85%] max-w-xs bg-white shadow-2xl transform transition-transform duration-300 lg:sticky lg:top-54 lg:h-[calc(100vh-13.5rem)] lg:translate-x-0 lg:shadow-none lg:w-64 lg:flex-none lg:border-r lg:border-gray-200 overflow-y-auto ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
          ...
          <div className="p-5 space-y-4">
            <h3 className="lg:hidden font-bold uppercase tracking-widest mb-6">Filters</h3>
            ...
            <button onClick={clearFilters} className="w-full py-2 text-xs font-bold text-red-600 border border-red-200 hover:bg-red-50 rounded-sm uppercase tracking-wider mt-4">Clear All Filters</button>
        ...
        <main className="flex-1 bg-white">
            <div className="p-4 md:p-8 min-h-[50vh]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    return (
                        <div key={product.id} className="group relative block bg-white">
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-gray-300 rounded-sm">
                    ...
                    <button onClick={clearFilters} className="bg-black text-white px-6 py-2 text-xs font-bold uppercase tracking-wider rounded-sm hover:bg-gray-800 transition-colors">Clear Filters</button>
                </div>
                )}
            </div>
        </main>
