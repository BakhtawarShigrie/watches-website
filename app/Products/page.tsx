...
import { brandsList, sidebarFiltersList } from "@/app/website-data";
...
export default function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [selectedBrand, sortOption, products]);
    ...
    <div className="bg-white min-h-screen font-sans text-[#333]">
      ...
      <div className="bg-[#f9f9f9] py-4 border-b border-gray-200">
        <div className="container mx-auto px-4 flex items-center justify-start md:justify-center gap-4 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <button className="hidden md:flex w-8 h-8 cursor-pointer flex-shrink-0 rounded-full bg-white shadow-sm items-center justify-center text-gray-400 hover:text-black">‹</button>
          ...
          <button className="hidden cursor-pointer md:flex w-8 h-8 flex-shrink-0 rounded-full bg-white shadow-sm items-center justify-center text-gray-400 hover:text-black">›</button>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div className="text-xs text-gray-500 font-medium">
            <Link href="/" className="hover:text-black">Home</Link><span className="mx-2">›</span><span>Watches</span><span className="mx-2">›</span><span>Mens Watches</span><span className="mx-2">›</span><span className="text-black font-bold">{selectedBrand === 'All' ? 'All Products' : selectedBrand}</span>
          </div>
          <div className="flex items-center justify-between w-full md:w-auto gap-6">
            ...
            <div className="flex items-center gap-6 text-sm ml-auto md:ml-0">
                ...
                <select value={sortOption} onChange={(e) => setSortOption(e.target.value)} className="appearance-none bg-transparent border-none font-bold text-black focus:ring-0 cursor-pointer pr-6">
                    <option value="default">Default Sorting</option>
                    <option value="lowToHigh">Price: Low to High</option>
                    <option value="highToLow">Price: High to Low</option>
                </select>
                <div className="absolute right-0 top-1/2 -translate-y-1/2 pointer-events-none"><svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1L5 5L9 1" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></div>
                </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 relative">
          ...
          {/* SIDEBAR */}
          <aside className={`fixed top-0 left-0 h-full w-[80%] max-w-xs bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:h-auto lg:w-64 lg:shadow-none lg:z-auto ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
            ...
            <div className="h-[calc(100%-130px)] overflow-y-auto lg:h-auto lg:overflow-visible p-5 lg:p-0">
                ...
                {sidebarFiltersList.map((filter, index) => (
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <svg className="w-3 h-3 text-black transform group-hover:rotate-180 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                    </div>
            ...
            <div className="absolute bottom-0 left-0 w-full p-4 border-t border-gray-200 bg-white lg:hidden">
                <button onClick={() => setIsSidebarOpen(false)} className="w-full bg-black text-white text-xs font-bold uppercase tracking-widest py-3 rounded-sm hover:bg-gray-800 transition-colors">View Results</button>
            </div>
          </aside>
      ...
      <footer className="bg-[#f9f9f9] text-[#333] pt-16 pb-8 font-sans border-t border-gray-200 mt-12">
        <div className="container mx-auto px-4">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="bg-black text-white px-4 py-2 font-serif text-xl tracking-widest border border-black cursor-pointer">LifeStyle <span className="block text-[8px] text-center tracking-[0.2em] -mt-1 text-gray-300">COLLECTION</span></div>
                <div className="text-xs text-gray-500 text-center md:text-left">Copyright 2024 Lifestyle Collection all rights reserved.</div>
                <div className="flex items-center gap-4">
                    ...
                    <button onClick={scrollToTop} className="bg-[#666] hover:bg-[#444] text-white w-10 h-10 flex items-center justify-center rounded-sm transition-colors shadow-sm"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 15.75 7.5-7.5 7.5 7.5" /></svg></button>
                </div>
            </div>
        </div>
      </footer>
       ...
       <button className="fixed bottom-6 right-6 z-50 h-12 w-12 bg-[#1a1a1a] rounded-full flex items-center justify-center hover:bg-[#333] transition-colors shadow-xl border border-zinc-800/50">
         <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-white"><path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a5.969 5.969 0 0 1-.474-.065 4.48 4.48 0 0 0 .978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" /></svg>
       </button>
